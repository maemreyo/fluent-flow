...
export class AIService {
  ...
  async generateSingleDifficultyQuestions(
    ...
    if (options?.customPrompt) {
      ...
      // Check if this is a Fill-in-the-Blank prompt
      isFillinBlankPrompt = customPrompt.exerciseType === 'fill_blank' || 
                           customPrompt.system_prompt.toLowerCase().includes('fill-in-the-blank') ||
                           customPrompt.system_prompt.toLowerCase().includes('fill in the blank')

      console.log(`📝 Exercise type detected: ${isFillinBlankPrompt ? 'Fill-in-the-Blank' : 'Multiple Choice'}`)
      ...
      const userPrompt = customPrompt.user_template
        ...
        .replace(/\{\{transcript\}\}/g, transcriptWithTimestamps)
    ...
    try {
      ...
      if (isFillinBlankPrompt) {
        // Handle Fill-in-the-Blank response format
        return this.processFillBlankResponse(parsedResponse, loop, difficulty, targetQuestionCount)
      } else {
        // Handle Multiple Choice response format (existing logic)
        return this.processMultipleChoiceResponse(parsedResponse, loop, difficulty, targetQuestionCount)
      }
    } catch (error: any) {
   ...
   */
  /**
   * Process Fill-in-the-Blank AI response
   */
  private processFillBlankResponse(
    parsedResponse: any,
    loop: SavedLoop,
    difficulty: 'easy' | 'medium' | 'hard',
  ...
  /**
   * Process Multiple Choice AI response (extracted from existing logic)
   */
  private processMultipleChoiceResponse(
    parsedResponse: any,
    loop: SavedLoop,
    difficulty: 'easy' | 'medium' | 'hard',
    targetQuestionCount: number
