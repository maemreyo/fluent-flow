...
export class UserService {
  ...
  async getUserVocabulary(
    ...
    let query = supabase.from('user_vocabulary_deck').select('*').eq('user_id', userId)
  ...
  async recordVocabularyReview(
    ...
    // Record the review
    const { error: reviewError } = await supabase.from('user_vocabulary_reviews').insert({
  ...
  // Delete vocabulary item from deck
  async deleteVocabularyFromDeck(vocabularyId: string): Promise<boolean> {
    if (!supabase) return false
    ...
    try {
      ...
      if (!currentUser) {
        console.warn('No current user found for deleting vocabulary')
        return false
      }

      console.log('Deleting vocabulary item:', vocabularyId, 'for user:', currentUser.id)

      const { error } = await supabase
        .from('user_vocabulary_deck')
        .delete()
        .eq('id', vocabularyId)
        .eq('user_id', currentUser.id) // Ensure user can only delete their own words

      if (error) {
        console.error('Error deleting vocabulary from deck:', error)
        return false
      }
