...
// New function to fetch current user's personal results
export const fetchUserResults = async (
  ...
  const response = await fetch(`/api/groups/${groupId}/sessions/${sessionId}/results?user=me`, {
    headers
  })

  if (!response.ok) {
    const errorData = await response.json()
    throw new Error(errorData.error || 'Failed to fetch user results')
  }

  const data = await response.json()
  return data.userResult || null
}